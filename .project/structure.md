...
- `src/posts.js`, `src/apps.js`: blog post (POSTS) and Works (APPS) metadata; Node-importable single sources shared by `src/data.js` (browser), the RSS feed generator, and the prerenderer.
- `src/data.js`, `src/articleBody.js`, `src/i18n.js`: site content and localized strings. `src/data.js` sources the About `PERSON` from `src/content/about.toml`.
- `src/content/theme.toml`: single source of truth for the light/dark color tokens (CSS custom properties), generated into CSS at build time. `src/content/about.toml`: bilingual About page content. Both are read by the `tomlContent()` plugin in `vite.config.js`.
