...
- `tests/`: verification and unit/smoke checks (`check_*.mjs`, `check_*.py`) plus their `fixtures/`, run through the verification phases.
...
- `tests/check_frontmatter.mjs`: metadata lint for every article Markdown file.
...
- `tests/check_metadata_quality.mjs`: static quality checks for generated tags and summaries.
- `tests/check_metadata_schema.mjs`: schema unit checks for valid and invalid metadata examples.
- `tests/check_generate_metadata.mjs`: metadata generation unit checks with a mock provider.
...
- `tests/check_pagefind_search.mjs`: smoke test for the built Japanese and English Pagefind indexes.
- `tests/check_prerendered_routes.mjs`: smoke test asserting every prerendered route/locale bakes its primary body content into `#root`.
...
- Search uses the generated Pagefind static index; keep `scripts/build_pagefind.mjs` and `tests/check_pagefind_search.mjs` aligned when indexed fields change.
