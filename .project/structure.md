...
- `scripts/make_ogp_placeholder.mjs`: dependency-free generator for the shared 1200x630 fallback image used by non-article routes.
...
- `npm run build` runs `vite build && node scripts/prerender.mjs && node scripts/build_pagefind.mjs` and produces a multi-file `dist/` (route directories + localized article images under `ogp/blog/` + `sitemap.xml` + `robots.txt` + `pagefind/`). Routing, links, metadata, and search indexes are path/locale based — see `.decisions/ogp-path-routing.md` and `.decisions/generated-article-ogp.md`.
