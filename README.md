...
| `npm run editor:build` | エディターを起動せずにビルドする |
...
Blog、Works、About の本文、公開範囲、削除状態、改訂履歴は Cloudflare D1 に保存し、本文画像とサムネイルは公開 URL を持たない Cloudflare R2 に保存します。
既存の公開コンテンツもすでに D1/R2 に保存されており、移行後の文書と画像は件数、バイト列、SHA-256 を移行元と照合しています。
...
初回だけ Tailscale Serve の管理権限を設定し、通常は次の順に起動します。
...
保存は新しい改訂を追加して現在の改訂を進め、古い画面からの保存は競合として止めます。
公開範囲や削除状態の変更は D1 に保存されますが、公開サイトへ反映されるのはエディターで公開操作を実行した後です。
公開操作は現在の改訂、公開範囲、削除状態を固定したジョブを作り、GitHub Actions には本文やタイトルではなく不透明なジョブ ID だけを渡します。
Actions は固定した D1/R2 スナップショットを一時領域へ取得し、メタデータ生成、翻訳、静的ビルド、検証、Cloudflare Pages への配信を順に実行します。
デプロイに成功した場合だけ公開中の改訂を更新するため、失敗したジョブが配信中のサイトを書き換えることはありません。

公開サイトは D1 をリクエスト時に参照せず、公開中かつ削除されていない項目から作った静的 HTML、Pagefind、RSS、サイトマップ、画像を配信します。
固定スナップショットの原稿は Git のコミット、再利用する Actions の成果物やキャッシュ、共有ログへ保存せず、公開用の静的ファイルだけを Pages へ渡します。

### エディターのセキュリティ境界

エディター本体はループバックアドレスだけで待ち受け、Tailscale Serve が認証付き HTTPS 接続を中継します。
Tailscale Funnel はエディターをインターネットへ公開するため使用せず、tailnet 側の grants または ACL でも接続できる利用者を執筆者に限定します。

エディター API は中継元、要求されたホスト、Tailscale が付与するログイン情報、状態変更リクエストの Origin を検査します。
ブラウザーには Cloudflare や GitHub の資格情報を渡さず、1Password から資格情報を受け取ったローカル Node.js サーバーだけが外部 API と通信します。

起動後は端末に表示された HTTPS URL をその場で開きます。
この URL、端末固有の接続名、tailnet のログイン情報、Cloudflare Access の資格情報、GitHub トークン、未公開原稿は、README、Issue、共有ログ、スクリーンショットへ記録しません。

Tailscale Serve を使えないときは、次のコマンドで同じ端末のループバック接続だけを許可します。

```sh
npm run editor -- --no-tailscale
```

公開処理とバックアップの詳しい境界は [.project/content-publication.md](.project/content-publication.md) と [.project/content-backup.md](.project/content-backup.md) に記載しています。
...
検証またはデプロイに失敗した場合は公開中の改訂を更新せず、直前の Pages デプロイを維持します。
...
Actions は日本語版を基に英語版を生成し、検証と Pages のデプロイが成功した場合だけ新しい改訂を公開します。
