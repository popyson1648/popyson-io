...
Blog、Works、About と画像は D1/R2 だけに保存され、リポジトリはコンテンツを持ちません。
ビルドは D1/R2 から取り出したスナップショットを読むため、その場所を `CONTENT_SNAPSHOT_ROOT` で指定しないと失敗します。
...
どちらの保存も非公開の改訂を追加するだけなので、公開操作を実行するまで公開中のサイトは変わりません。
...
エディターは起動時に資格情報を読み込み、動作中は読み直しません。
...
├── archive/                        # 保管しているだけのコンテンツ
│   └── content/                    # 記事、制作物、About、サムネイル。ビルドもテストも読まない
