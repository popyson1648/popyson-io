...
編集は入力が止まってから 1.5 秒後に自動保存され、`Ctrl+S` と `Cmd+S` でも保存できます。
保存はどちらも D1 への改訂追加なので、明示的に保存しないまま失う下書きはありません。
残らないのは自動保存が走る前にブラウザーを閉じた分と、保存が失敗したあとに続けた編集だけです。
自動保存はどちらも非公開の改訂として積まれるだけで、公開操作を実行するまで公開サイトには反映されません。
...
資格情報は `op run` が起動時に一度だけ解決してプロセスへ渡します。
そのため 1Password のセッションが編集中に失効しても、起動済みのエディターは保存も公開も続けられます。
失効が影響するのは次にエディターを起動するときで、Cloudflare Access のサービストークンと GitHub の個人アクセストークンはこれとは別に固有の有効期限を持ちます。
