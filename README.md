...
- **記事表示**：目次、関連記事、Shiki によるコードハイライト、コードのコピー操作、スライドと動画と投稿の埋め込みを備えます。
...
ビルドは、`CONTENT_SNAPSHOT_ROOT` で指定した D1/R2 のスナップショットを読みます。
...
`Ctrl+S` と `Cmd+S` でもその場で保存できます。
どちらの保存も非公開の改訂を追加し、公開中のサイトは公開操作を実行したときに変わります。
...
エディターは起動時に読み込んだ資格情報を、動作中はそのまま使い続けます。
そのため 1Password のセッションが編集中に失効しても、保存と公開は続けられます。
...
│   ├── embedFrames.js              # 投稿の埋め込みの高さとテーマを合わせる処理
│   ├── i18n.js                     # UI 文言の日英辞書
...
│   ├── fixtures/                   # テストが読む入力例と、テスト専用のコンテンツ
...
各 HTML には実際の本文が含まれるため、HTML だけを読むクローラーもページの本文と URL 固有のメタデータを取得できます。
...
ブラウザーでの画面遷移とビルド時の事前描画が同じ関数を使うため、初期 HTML と遷移後の `<head>` は同じ規則に従います。

### ビルド時に完結する記事処理

記事の Markdown は Unified の処理系で HTML と検索用テキストへ変換します。
Shiki はライト用とダーク用の色を一度に埋め込み、構文解析はビルド時に終わります。
外部リンクには `rel="noreferrer"` を付け、画像は遅延読み込みにし、リンクと画像は許可した URL スキームだけを出力します。

### 静的ファイルだけで動く全文検索

Pagefind の索引は日英の記事ごとにビルド時に生成します。
検索時には現在の言語だけを対象にし、タイトル、概要、タグ、本文から一致箇所を探します。
索引と JavaScript を配信するだけで全文検索が動きます。
...
CI の検査はスナップショット内のファイルだけを読み、自動項目がすべて解決済みであることを確認します。
...
行頭に `|` を置くと、その行は直前の行から改行して表示され、行間は文章が折り返したときと同じ幅になります。
空行で分けた段落の間隔よりも狭く2行を並べたいときに使います。
`|` とその直後の空白は表示から取り除き、行の残りは通常の Markdown として扱います。
...
この記法は引用、リスト、コールアウトの中でも同じように動きます。
行頭に文字としての `|` を表示する場合は `\|` と書き、2行は1行に続けて表示されます。
...
同じ見出しが複数ある場合は、それぞれに一意のリンク ID を割り当てます。
...
チェックボックスは操作を受け付けない表示用の要素として出力され、記事本文に書いた状態をそのまま示します。
...
補足を開閉できる状態で置く場合は、`details` コンテナーディレクティブを使います。
...
タイトルを省略すると、日本語では「詳細」、英語では「Details」と表示します。
最初から本文を開く場合は、`:::details[補足のタイトル]{open}` と書きます。
エディターでは `/折りたたみ` コマンドから記法を挿入できます。

### 埋め込み

スライド、動画、SNS の投稿はリーフディレクティブ `::embed` で埋め込みます。
サービスのページ URL を `url` に書くと、そのサービスが公開している iframe の URL へ変換します。
...
::embed[キャプション]{url="https://youtu.be/dQw4w9WgXcQ"}
```

角括弧のラベルは省略でき、書いた場合は iframe のタイトルと、元のページへ向かうキャプションのリンクになります。

| サービス | 書ける URL |
| --- | --- |
| YouTube | `youtu.be/<id>`、`watch?v=<id>`、`shorts/<id>`、`live/<id>`、`embed/<id>` |
| Docswell | `/s/<user>/<id>-<slug>`、`/slide/<id>` |
| Speaker Deck | 埋め込みコードの `/player/<id>` |
| Vimeo | `vimeo.com/<id>` |
| X | `x.com/<user>/status/<id>`、`twitter.com` と `mobile.` の同じパス、`/i/web/status/<id>`、`/statuses/<id>` |
| Instagram | `/p/<code>`、`/reel/<code>`、`/tv/<code>`（先頭に `/<user>` が付く形も可） |

YouTube は `t` と `start` に書いた再生開始位置を秒に直して引き継ぎ、`youtube-nocookie.com` から配信します。
Speaker Deck は再生用の ID が埋め込みコードにだけ現れるため、そこにある `/player/<id>` の URL を貼ります。
X は `platform.twitter.com` から、追跡を抑える `dnt` を付けて配信します。
Instagram は公開されている投稿を表示し、それ以外はログインの案内を表示します。

表にあるサービスの `http` と `https` の URL を埋め込みへ変換し、それ以外の URL と記法の誤りは通常のリンクとして表示します。
URL を1行に貼っただけのときも、リンクとして表示します。

スライドと動画の枠は 16:9 で表示し、遅延読み込みします。
投稿の高さは内容によって変わるため、X と Instagram の枠は表示された投稿が伝えてきた実寸に合わせます。
X の投稿はサイトのテーマに合わせて明暗を切り替えます。
検索の索引には本文の文章だけが入ります。
`scripts/embedProviders.mjs` に変換の定義を1つ加えると、対応するサービスが増えます。

### HTML と URL の制限

リンクと画像には、`http`、`https`、`mailto`、ページ内リンク、サイト内の絶対パス、相対パスを使用できます。
これ以外の URL を指定すると、リンクは表示テキストだけになり、画像は取り除かれます。

```markdown
[安全なリンク](https://example.com)
[表示テキストだけが残るリンク](javascript:alert(1))

![安全な画像](/images/example.png)
![取り除かれる画像](data:text/html;base64,...)
```

Markdown に書いた raw HTML は、タグを取り除いて内側の文章だけを残します。

```markdown
<strong>文字として残るタグ</strong>
```

この制限は、サービスが配布する `<script>` 付きの埋め込みコードにも及びます。
スライド、動画、SNS の投稿は[埋め込み](#埋め込み)の記法で貼ります。

### 脚注

脚注は GitHub と同じ記法で書き、参照番号と本文末尾の一覧に変換します。
...
定義リストと数式は、書いた記号をそのまま含む段落として出力します。
Mermaid はコードブロックとして表示します。
これらを図や専用の表示として扱う場合は、Markdown 変換処理とテストを追加します。
...
推奨サイズを表示幅のおよそ2倍にしているのは、画素密度の高い画面でも鮮明に表示するためです。
