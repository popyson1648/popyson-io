...
共有するときのページ URL を `url` に書くと、サービスが公開している iframe の URL へ変換します。
...
角括弧のラベルは省略できます。
書いた場合は iframe のタイトルと、元のページへ向かうキャプションのリンクになります。
...
YouTube は `t` と `start` の値を秒に直して引き継ぎ、`youtube-nocookie.com` から配信します。
Speaker Deck は発表ページの URL に再生用の ID が含まれないため、埋め込みコードの URL を貼ります。
対応していないサービスの URL、`http` と `https` 以外の URL、記法の誤りは、空の枠ではなく通常のリンクになります。
URL を1行に貼っただけのときも、埋め込まずリンクとして表示します。
枠は 16:9 で表示し、遅延読み込みします。
埋め込みは検索の索引に含めません。
対応サービスを増やすときは `scripts/embedProviders.mjs` に1件追加します。
...
サービスが配布する `<script>` 付きの埋め込みコードも、この制限によって記事には残りません。
