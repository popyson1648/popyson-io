...
スラッグはディレクトリ名であり、そのまま URL になります。
...
使える文字は**英小文字・数字・ハイフン**で、先頭は英小文字か数字です（`^[a-z0-9][a-z0-9-]*$`）。
大文字やスペースは受け付けません。
制作物の名前そのものは front matter の `title` に書くので、スラッグとは別で構いません（`LineWatch` に対して `linewatch`）。
...
`thumbnail` と `hero` は表示する場所が違います。

| 項目 | 表示場所 | 表示幅（1280px 時） | 推奨サイズ |
| --- | --- | --- | --- |
| `thumbnail` | 一覧カードの上部 | 535px | **1200 × 675** |
| `hero` | 詳細ページのタグラインの下 | 820px | **1600 × 900** |

どちらの枠も **16 : 9** です。高さはピクセルで固定せず幅から比率で決まるので、16:9 の画像を置けば切り取られずに収まります。画面の解像度が高い環境を考えて、表示幅のおよそ2倍を推奨サイズにしています。

比率の違う画像を置いても表示は壊れません。`object-fit: cover` で枠を埋めるように中央から切り取られます。両方とも 16:9 なので、1枚の画像を両方に指定することもできます。
...
先頭は必ず `/` で始めます（`//` で始まる形は外部ホストとして解釈されるため受け付けません）。
空文字にすると、その枠にはプレースホルダが表示されます。片方だけ指定することもできます。
