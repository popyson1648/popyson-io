...
export function L(obj, lang) {
  // localize {ja,en} or plain string
  if (obj == null) return "";
...
export function bodyText(id, lang) {
  ...
  if (body && !Array.isArray(body)) {
    ...
    return String(
      typeof localizedBody === "string" ? localizedBody : localizedBody.text || "",
    ).toLowerCase();
  }
  const blocks = Array.isArray(body) ? body : [];
  const parts = [];
  for (const b of blocks) {
    if (b.kind === "p" || b.kind === "h2" || b.kind === "msg")
      parts.push(L({ ja: b.ja, en: b.en }, lang));
...
export const Icon = {
  ...
  github: (p) => (
    <svg {...sIcon} fill="currentColor" stroke="none" {...p}>
  ...
  rss: (p) => (
    <svg {...sIcon} {...p}>
      <path d="M5 19a1 1 0 1 0 0-2 1 1 0 0 0 0 2zM5 12a7 7 0 0 1 7 7M5 5a14 14 0 0 1 14 14" />
    </svg>
  ),
  check: (p) => (
    <svg {...sIcon} {...p}>
  ...
  xcom: (p) => (
    <svg {...sIcon} width={15} height={15} fill="currentColor" stroke="none" {...p}>
...
export function Chip({ children, on = false, onClick = undefined, isStatic = false }) {
  ...
  return (
    <button
      className={"chip" + (on ? " on" : "") + (isStatic ? " static" : "")}
      onClick={onClick}
      type="button"
      aria-pressed={!!on}
    >
...
export function TopBar() {
  ...
  const link = (to, key) => (
    <a
      href={localized(to, lang)}
      className={here === key ? "active" : ""}
      data-nav-key={key}
        ...
        <div className="topbar-tools">
          <div className="tool-group tool-group-actions" aria-label={t.tools}>
            ...
            <Dropdown
              width={176}
              align="right"
              button={({ open, menuId, toggle }) => (
                <button
                  className={"icon-btn theme-trigger" + (open ? " active" : "")}
                  type="button"
                  onClick={toggle}
                  aria-expanded={open}
...
function searchSnippet(text, q, max = 112) {
  ...
  const start = Math.max(
    0,
    Math.min(center - Math.floor(max / 2), Math.max(0, value.length - max)),
  );
